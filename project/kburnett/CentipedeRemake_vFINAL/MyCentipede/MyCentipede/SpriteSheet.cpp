...
SpriteSheet::SpriteSheet(sf::Texture& tex, int CellCountHor, int CellCountVert)
{
	...
	pGlyphFactory = new GlyphFactory();
}
...
{
	return pGlyphFactory->CreateGlyph(this, CharToIndex(c), pos);
	//return new Glyph(this, CharToIndex(c), pos);	// as an extra efficiency, we should add a glyph recycling factory to use here
}
...
void SpriteSheet::Recycle(Glyph* g)
{
	pGlyphFactory->RecycleGlyph(g);
}

SpriteSheet::~SpriteSheet()
{
	delete pGlyphFactory;
}